...
export const miscTools = [
  ...
  },
  {
    ...
    parameters: {
      cmd: { type: 'object', description: 'Объект команд: { "key1": "crm.deal.get?id=1", "key2": "crm.contact.get?id=2" }' },
...
export const buildMiscRequest = (toolName, args) => {
  switch (toolName) {
    ...
    case 'bitrix_batch': {
      const cmd = ensureObject(args.cmd, 'cmd must be an object');
    ...
    case 'bitrix_telephony_call_list': {
      ...
      const payload = { FILTER: filter, START: start };
      if (sort) {
        payload.SORT = sort;
      }
      if (order) {
