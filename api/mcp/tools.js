...
const ensureNonEmptyObject = (value, message) => {
  const objectValue = ensureObject(value, message);
...
const normalizeCallPayload = (toolOrPayload, args) => {
...
export const buildBitrixRequest = (toolOrPayload, args = {}) => {
  ...
  if (tool === 'bitrix_update_deal') {
    const id = ensurePositiveNumber(normalizedArgs.id, 'Parameter "id" must be a positive number');
    const fields = ensureNonEmptyObject(normalizedArgs.fields, 'Parameter "fields" must be a non-empty object');
