...
export const buildBitrixRequest = (toolName: string, args: Record<string, unknown> = {}) => {
  ...
  if (toolName === 'bitrix_search_deals') {
    const filter =
      ensureObject<Record<string, unknown>>(normalizedArgs.filter, 'Parameter "filter" must be an object when provided') || {};
  ...
  if (toolName === 'bitrix_get_contact') {
    const id = ensurePositiveNumber(normalizedArgs.id, 'Parameter "id" must be a positive number');
    return { method: 'crm.contact.get', payload: { id } } as const;
  }
  ...
  if (toolName === 'bitrix_get_company') {
    const id = ensurePositiveNumber(normalizedArgs.id, 'Parameter "id" must be a positive number');
    return { method: 'crm.company.get', payload: { id } } as const;
  }
  ...
  if (toolName === 'bitrix_get_lead') {
    const id = ensurePositiveNumber(normalizedArgs.id, 'Parameter "id" must be a positive number');
    return { method: 'crm.lead.get', payload: { id } } as const;
  }
