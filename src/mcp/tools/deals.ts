...
export const dealTools: ToolDefinition[] = [
  ...
  },
  {
    ...
    parameters: {
      fields: {
        type: 'object',
        description: 'Поля сделки (обязательно TITLE, опционально OPPORTUNITY, STAGE_ID и др.)',
      },
    },
  },
  {
  ...
  },
  {
    ...
    parameters: {
      fields: {
        type: 'object',
        description: 'Поля для создания (обязательно FIELD_NAME, USER_TYPE_ID)',
      },
    },
  },
];

export const buildDealRequest = (toolName: string, args: Record<string, unknown>) => {
  switch (toolName) {
